...
extension SceneLocationView {
  ...
  func addAnnotation(_ annotation: MKAnnotation) {
    guard let altitude = currentLocation()?.altitude else {
      return
    }
...
extension LocationNode {
  static func create(polyline: MKPolyline, altitude: CLLocationDistance) -> [LocationNode] {
    ...
    let lightNode = makeSCNLight(type: .ambient, intensity: 25, position: SCNVector3(x: 0, y: 10, z: 0))

    let lightNode3 = makeSCNLight(type: .omni, intensity: 100, position: SCNVector3(x: -10, y: 10, z: -10))
  ...
  private static func makeSCNLight(type: SCNLight.LightType, intensity: CGFloat, position: SCNVector3) -> SCNNode {
